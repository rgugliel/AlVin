...
bool RCyclotomic7Integer_AlVin::PreRun() { return true; }

RCyclotomic7Integer_AlVin::RCyclotomic7Integer_AlVin(
...
void RCyclotomic7Integer_AlVin::print_initialInformationChild() const {
  ...
  if (strOuputMathematicalFormat == "mathematica") // quadratic form
  {
    cout << "l1 := 2*Cos[2*\\[Pi]/7];\nl2 := 2*Cos[4*\\[Pi]/7];\nl3 := "
            "2*Cos[6*\\[Pi]/7];\nRCI7[l_] := l[[1]] * l1 + l[[2]] * l2 + "
            "l[[3]] * l3;"
         << endl;
  } else if (strOuputMathematicalFormat == "pari") {
    cout << "RCI7 = (x,y,z) -> 2*( x*cos(2*Pi/7) + y*cos(4*Pi/7) + "
            "z*cos(6*Pi/7) );"
         << endl;
  }
}
...
void RCyclotomic7Integer_AlVin::findVector(
  ...
  RCyclotomic7Integer rciSumComp_conj2(rciSumComp),
      rciSumComp_conj3(rciSumComp);
  ...
  bool bR1IsAnInt(gaol_R1.is_an_int()), bR2IsAnInt(gaol_R2.is_an_int()),
      bR3IsAnInt(gaol_R3.is_an_int());
  bool bRiAreLongInt(bR1IsAnInt && bR2IsAnInt && bR3IsAnInt);
  long int iR1(bR1IsAnInt ? floor(gaol_R1.left()) : 0),
      iR2(bRiAreLongInt ? floor(gaol_R2.left()) : 0),
      iR3(bRiAreLongInt ? floor(gaol_R3.left()) : 0);
  ...
  interval gaol_min(
      gaol_temp + gaol_R1 * (gaol_2 * (gaol_l1 + gaol_l3) + gaol_3 * gaol_l2));
  ...
  for (long int b(floor(gaol_min.left())); b <= bMax && !bReturn; b++) {
    ...
    if (bRiAreLongInt) {
      ...
      if (iC0 == iC1 && iC1 && iC2 && !(iC0 % 7)) // the result is an int
        ...
        gaol_min = interval(iC0) * gaol_l1 + interval(iC1) * gaol_l2 +
                   interval(iC2) * gaol_l3;
        gaol_min = ceil(gaol_min / gaol_7);

        if (!gaol_min.is_an_int())
          throw(string("RCyclotomic7Integer_AlVin::findVector() : Not an int "
                       "(bound inf for a)"));
      ...
      if (iC0 == iC1 && iC1 && iC2 && !(iC0 % 7)) // the result is an int
        ...
        gaol_max = interval(-iC0) * gaol_l1 + interval(-iC1) * gaol_l2 +
                   interval(-iC2) * gaol_l3;
        gaol_max = floor(gaol_max / gaol_7);
      ...
      gaol_min *= ((gaol_2 * (gaol_l1 + gaol_l3) + gaol_l2 * gaol_3) * gaol_b +
                   gaol_l3 * gaol_R2 - gaol_R3 - gaol_R1);
      gaol_max *= ((gaol_2 * (gaol_l1 + gaol_l3) + gaol_l2 * gaol_3) * gaol_b -
                   gaol_l3 * gaol_R2 + gaol_R3);
...
void RCyclotomic7Integer_AlVin::findPossibleNorms2() {
  ...
  rciFactors = vector<RCyclotomic7Integer>(
      rciFactors.begin(), unique(rciFactors.begin(), rciFactors.end()));
