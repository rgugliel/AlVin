...
{
	public:
		array< mpz_class, 3 > iC; ///< Decomposition in the basis lambda_1, ..., lambda_(iDegree)
		
	private:
		static bool bClassInitialized; ///< If the class has been initialized
		
		static vector< vector< int > > iPowersL; ///< Decomposition of l^2, l^3, ..., l^iDegree
		static array< array< array<mpq_class, 2>, 3 >, 2 > mpqRationalApproximations; ///< Rational approximations (10^-50, 10^-200)
	...
	private:
		static void computePrimesDecomposition_factorMinimalPolynomial( GEN& gFactors, const unsigned int& iPrime );
		static void computePrimesDecomposition_addFactor( const unsigned int& iPrime, const unsigned int& iFactorsCount, RCyclotomic7Integer rci );
};
...
inline interval gaol_SQRTQuotient( RCyclotomic7Integer rciNum, const RCyclotomic7Integer& rciDenom )
{
