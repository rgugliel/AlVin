...
RationalInteger_InfiniteNSymetries::RationalInteger_InfiniteNSymetries( AlVin* alvin )
: InfiniteNSymetries( alvin )
{
	...
	riVectorsProducts = vector< vector< RationalInteger > >( iVectorsCount, vector< RationalInteger >( iVectorsCount, 0 ) );
	
	// -------------------------------------------------
	// Quadratic form
	for( auto ai : aiQF )
		rriQF.push_back( Rational<RationalInteger>( *dynamic_cast< RationalInteger* >( ai ) ) );
	...
	for( i = 0; i < iVectorsCount; i++ )
	{
		...
		for( j = 0; j <= iDimension; j++ )
			v.push_back( Rational<RationalInteger>( *dynamic_cast< RationalInteger* >( aiVectors[i][j] ) ) );
		
		v2 = Matrix< Rational<RationalInteger>, 1, Dynamic >::Map( &v[0], 1, iDimension+1 );
	...
	for( i = 0; i < iVectorsCount; i++ )
	{
		...
		for( j = i+1; j < iVectorsCount; j++ )
		{
			...
			riVectorsProducts[i][j] = riVectorsProducts[j][i] =  *dynamic_cast< RationalInteger* >( aiProd );
			delete aiProd;
		}
	}
	...
	riDottedWeights = vector< vector< Rational<RationalInteger> > >( iVectorsCount, vector< Rational<RationalInteger> >( iVectorsCount, 0 ) );
	for( i = 0; i < iVectorsCount; i++ )
	{
		for( j = i + 1; j < iVectorsCount; j++ )
		{
			if( iGraphMatrix[i][j] == 2 ) // dotted
			{
				...
				riDottedWeights[i][j] = riDottedWeights[j][i] = Rational<RationalInteger>( *dynamic_cast< RationalInteger* >( aiNum ), *dynamic_cast< RationalInteger* >( aiNorm1 ) );
...
Rational< RationalInteger > RationalInteger_InfiniteNSymetries::rriVectorNorm( const Matrix< Rational< RationalInteger >, -1, 1 >& rriV ) const
{
	if( rriV.rows() != iVectorSize )
		throw( string( "RationalInteger_InfiniteNSymetries::rriVectorNorm: Invalid size" ) );
...
Rational<RationalInteger> RationalInteger_InfiniteNSymetries::rriVectorsProduct(const Matrix<Rational<RationalInteger>, Dynamic, 1>& rriV1, const Matrix<Rational<RationalInteger>, Dynamic, 1>& rriV2) const
{
...
vector< GraphInvolution > RationalInteger_InfiniteNSymetries::FindIsomorphismsInSubgraph( const vector< unsigned int >& iVertices )
{
	...
	FullPivLU<Matrix< Rational<RationalInteger>, Dynamic, Dynamic >> lu_mVectorsVertices_temp(mVectorsVertices_temp);
	...
	for( i = 0; i < iVerticesCount; i++ )
	{
		VECTOR(verticesColors)[i] = riVectorsProducts[iVertices[i]][iVertices[i]].iVal;
	...
	for( int is( 1 ) ; is < iIsomorphismsCount; is++ ) // Don't consider the identity
	{
		...
		Matrix< Rational<RationalInteger>, Dynamic, Dynamic > iIntegralSymmetry; ///< Permutation matrix wrt to a basis contained in the vectors corresponding to iVertices
		Matrix< Rational<RationalInteger>, Dynamic, Dynamic > mVectorsVerticesBasisInv( mVectorsVerticesBasis.inverse() );
		
		iIntegralSymmetry.resize( iVectorSize, iVectorSize );
		for( i = 0; i < iVectorSize; i++ )
			iIntegralSymmetry.col(i) = mVectorsVerticesBasisInv * rriVectorsC[iVertices[iPermutation[iLinearlyIndependantVertices[i]]]];
		
		Matrix< Rational<RationalInteger>, Dynamic, Dynamic > mCanBasis( mVectorsVerticesBasis * iIntegralSymmetry * mVectorsVerticesBasisInv );
...
bool RationalInteger_InfiniteNSymetries::FindIntegralSymmetryFromSubgraph( const vector< unsigned int >& iVertices )
{
	unsigned int i, j;
...
bool RationalInteger_InfiniteNSymetries::bDottedSameWeight( const unsigned int& v1, const unsigned int& v2, const unsigned int& w1, const unsigned int& w2 ) const
{
	return ( riDottedWeights[v1][v2] == riDottedWeights[w1][w2] );
}

void RationalInteger_InfiniteNSymetries::WorkWithIsomorphisms( const vector< unsigned int >& iVertices, const vector< GraphInvolution >& iIsomorphisms )
{
	...
	for( auto iIso : iIsomorphisms )
	{
		...
		for( i = 0; i < iVerticesCount && j < iIso.iOrbitCount; i++ )
		{
			if( iIso.iPermutation[i] == i )
				...
				m.col(j++) = rriVectorsC[iVertices[i]] + rriVectorsC[ iVertices[iIso.iPermutation[i]]];
		}
		
		// We get a basis of the space of fixed points
		FullPivLU<Matrix< Rational<RationalInteger>, Dynamic, Dynamic >> lu(m);
		Matrix< Rational<RationalInteger>, Dynamic, Dynamic > basisFixedPoints( lu.image(m) );
		...
		#pragma omp critical
		{
			...
			else // We already have fixed points
			{
				...
				Matrix< Rational<RationalInteger>, Dynamic, Dynamic > tempVectors( rriBasisFixedPoints );
				...
				FullPivLU<Matrix< Rational<RationalInteger>, Dynamic, Dynamic > > luTemp(tempVectors);
				Matrix< Rational<RationalInteger>, Dynamic, Dynamic > ker( luTemp.kernel() );
				...
				if( ker.cols() < iFixedPointsDimension && !bFinished ) // If we earned something
				{
					...
					{
						Matrix< Rational<RationalInteger>, Dynamic, 1 > v( ker(0,j)*tempVectors.col(0) );
					...
					else if( iFixedPointsDimension == 2 )
					{
						...
						if( rriVectorNorm( rriBasisFixedPoints.col(0) ) >= 0 && rriVectorNorm( rriBasisFixedPoints.col(1) ) >= 0 && rriVectorsProduct( rriBasisFixedPoints.col(0), rriBasisFixedPoints.col(1) ) == 0 )
							bFinished = true;
					}
				}
			}
...
void RationalInteger_InfiniteNSymetries::print_basisFixedPoints( const string& strSpacer ) const
{
	...
	for( unsigned int i(0); i < iFixedPointsDimension; i++ )
		cout << strSpacer << Matrix<Rational<RationalInteger>, 1, Eigen::Dynamic>( rriBasisFixedPoints.col(i).transpose() ) << endl;
}
