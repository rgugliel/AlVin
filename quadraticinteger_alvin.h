...
{
	private:
		vector< QuadraticInteger > qiQF; ///< Local copy of the coefficients of the quadratic form; used to work faster
		...
		vector< QuadraticInteger > qiDQFConj; ///< Conjugates of coefficients of the quadratic form * d
		
		vector< QuadraticInteger > qiBilinearProducts; ///< To control during findVector that the vector has negative product with the preceding ones
	...
	public:
		QuadraticInteger_AlVin( const vector< QuadraticInteger >& iQuadraticFormCoeffs, const string &strOuputMathematicalFormat, const bool& bWriteInfo, const bool& bDebug );
	...
	private:
		...
		/*!	\fn findVector( QuadraticInteger* qi0, QuadraticInteger* qiNorm2, unsigned int iIndex, QuadraticInteger qiSumComp, QuadraticInteger qiGCDComponents );
		 ...
		 * 	\param qiSumComp( const unsigned int& ): iSumComp = \sum_{i=iIndex}^iDimension a_i * x_i^2
		 * 	\param qiGCDComponents( QuadraticInteger ): GCD of the found components
		 */
		void findVector( QuadraticInteger* qi0, QuadraticInteger* qiNorm2, unsigned int iIndex, QuadraticInteger qiSumComp, QuadraticInteger qiGCDComponents );
