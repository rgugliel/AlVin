...
namespace Eigen 
{
	...
	template<> struct NumTraits<Rational<RationalInteger>> ///< \struct NumTraits<Rational<RationalInteger>> rationalinteger_infinitensymetries.h "Specialization for Eigen matrices to RationalInteger type"
	{
	...
	namespace internal {
		template<>
		struct significant_decimals_impl<Rational<RationalInteger>> ///< \struct significant_decimals_impl<Rational<RationalInteger>> rationalinteger_infinitensymetries.h "Specialization for Eigen matrices to RationalInteger type"
		{
...
{
	private:
		vector< Matrix< Rational<RationalInteger>, Dynamic, 1 > > rriVectorsC; ///< Vectors found by AlVin, in column
		vector< vector< Rational<RationalInteger> > > riDottedWeights; ///< Weights of the dotted lines
		
		Matrix< Rational<RationalInteger>, Dynamic, Dynamic > rriBasisFixedPoints; ///< Basis of the fixed points of the involutions, in column
		
		vector< Rational<RationalInteger> > rriQF; ///< Coefficients of the diagonal quadratic form (in fractions)
		vector< vector< RationalInteger > > riVectorsProducts; ///< Products of each pair of vecteurs
