...
QuadraticInteger_VFs::QuadraticInteger_VFs( vector< AlgebraicInteger* > aiPossibleNorms2, const QuadraticInteger& qiAlpha0 )
: qiAlpha0( qiAlpha0 ), AlVinFractions( aiPossibleNorms2 ), qiPossibleNorms2_max( dynamic_cast< QuadraticInteger* >( aiPossibleNorms2[ aiPossibleNorms2.size() - 1 ] ) )
{
	iBatchSize = 1;
}
...
void QuadraticInteger_VFs::computeNextAlVinFractions()
{
	...
	for( auto aiE : aiPossibleNorms2 )
	{
		...
		QuadraticInteger qiMinEpsilon( *qiE ), qiMaxEpsilon( *qiE ), qiConjEpsilon( *qiE );
		...
		if( QuadraticInteger::bIsOneMod4 )
		{
			...
			iYMax = iSqrtEp + QuadraticInteger::iSQRT_quotient( qiMaxEpsilon, QuadraticInteger( QuadraticInteger::d ) ) + 1;

			// isqrt( iMin * epsilon / d )
			iYMin = QuadraticInteger::iSQRT_quotient( qiMinEpsilon, QuadraticInteger( QuadraticInteger::d ) ) - iSqrtEp;
			
			qiTemp.set( &qiMinEpsilon );
			qiTemp.multiplyBy( 4 );
			
			long int iSQRTi4MinEpsilon( QuadraticInteger::iSQRT_quotient( qiTemp, QuadraticInteger( 1 ) ) );
			
			long int iSQRTMaxEpsilon( QuadraticInteger::iSQRT_quotient( qiMaxEpsilon, QuadraticInteger( 1 ) ) );
			...
			for( y = iYMin; y <= iYMax; y++ )
			{
				...
				{
					QuadraticInteger qi( QuadraticInteger( x, y ) ), qiSquare( QuadraticInteger( x, y ) );
					...
					AlVinFraction* vf = new AlVinFraction( new QuadraticInteger( qi ), new QuadraticInteger( *qiE ), qiNum );
		...
		else
		{
			...
			long int iSQRTsupConjEpsilonMinConjAlpha0( QuadraticInteger::iSQRTsup_quotient( qiConjEpsilon, qiMinConjAlpha0 ) );
			
			iTemp = QuadraticInteger::iSQRT_quotient( qiMinEpsilon, QuadraticInteger( 1 ) ) - iSQRTsupConjEpsilonMinConjAlpha0;
			iXMin = (iTemp % 2) ? iTemp / 2 + 1 : iTemp / 2; // ceil
			iXMax = ( QuadraticInteger::iSQRTsup_quotient( qiMaxEpsilon, QuadraticInteger( 1 ) ) + iSQRTsupConjEpsilonMinConjAlpha0 ) / 2;
			
			for( x = iXMin; x <= iXMax; x++ )
			{
				iTemp = iSQRTQuotient( (unsigned long int)x * x, (unsigned long int) QuadraticInteger::d );
				iYMin = QuadraticInteger::iSQRT_quotient( qiMinEpsilon, QuadraticInteger( QuadraticInteger::d ) ) - iTemp;
				iYMax = QuadraticInteger::iSQRT_quotient( qiMaxEpsilon, QuadraticInteger::d ) - iTemp;
