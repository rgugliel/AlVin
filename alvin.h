...
class AlVin
{
	protected:
		...
		bool bComputeInvariantsPolyhedron; ///< If yes, we compute the invariants of the final polyhedron
		bool bDebug; ///< If true, more information are displayed
		bool bWriteInfo; ///< If we want to write informations (false if AlVin is used "as a plugin")
		...
		vector< unsigned int > iComponentLessThan; ///< The entry [ i ] = j means x_i <= x_j, [ i ] = 0 means no information for x_i
		...
		vector< vector< AlgebraicInteger* > > aiVectors_candidates; ///< Vectors which are compatiable with the previous
		
		vector< vector< unsigned int > > iCoxeterMatrix;

		vector< AlgebraicInteger* > iBilinearProducts; ///< To control during findVector_simple that the vector has negative product with the preceding ones
	...
	public:
		/*!	\fn AlVin( const string& strOuputMathematicalFormat, const bool& bWriteInfo, const bool& bDebug )
		 * 	\brief Constructor for the parent AlVin object
		 * 	\param strOuputMathematicalFormat( const string& ): Format for the output: generic, mathematica, pari, latex
		 * 	\param bWriteInfo( const bool& ): If true, display info
		 * 	\param bDebug( const bool& ): If true, some debug info
		 */
		...
		/*!	\fn Run( unsigned int iMinVectors = 0, unsigned int iMaxVectors = 0, bool bLastCheckFV = true )
		 * 	\brief Run the algo!
		 * 	\param iMinVectors(unsigned int): If specified, does not check the finiteness while this->iVectorsCount < iMinVectors
		 * 	\param iMaxVectors(unsigned int): If specified, stops when this->iVectorsCount == iMaxVectors
		 * 	\param bLastCheckFV(bool): If false, does not check the finiteness for the final polyhedron (useful for debug)
		 */
		bool Run( unsigned int iMinVectors = 0, unsigned int iMaxVectors = 0, bool bLastCheckFV = true );
		
		/*!	\fn AlgebraicInteger* aiBilinearProduct( const vector< AlgebraicInteger* >& v1, const vector< AlgebraicInteger* >& v2 )
		 * 	\brief Compute the product between two vectors
		 */
		 ...
		 * 	\brief Return the list of possible values for (e,e), where e is a root of the quadratic lattice
		 */
		vector< AlgebraicInteger* > get_aiPossibleNorm2() const;
		
		/*!	\fn get_ptraiPossibleNorm2
		 * 	\brief Return a pointer to the list of possible values for (e,e), where e is a root of the quadratic lattice
		 */
		 ...
		 * 	\brief Create image with the graph: 1: forced yes, 0: forced no, -1: default value (only if at most 25 vectors found)
		 */
