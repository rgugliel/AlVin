...
void NotReflective::prepareGraphsList()
{
	...
	vector< vector< short unsigned int > > iGraphsNotExtendable( ci.bCanBeFiniteCovolume_complete() );
	...
	for( auto& iGVertices : iGraphsNotExtendable )
	{
		...
		for( vector<short unsigned int>::reverse_iterator it( iGVertices.rbegin() ); it != iGVertices.rend(); ++it )
		{
			if( *it >= iDimension ) // Intersting vertices are 0, ..., iDemension - 1
				continue;
			
			if( *it == iDimension - 1 || *aiQF[*it + 1] != *aiQF[*it + 2] )
				iVariablesName[*it + 1] = 0; // This coefficient of the vector is really 0
			else
		...
		iVariablesNameDistinct = vector< short unsigned int >( iVariablesNameDistinct.begin(), unique( iVariablesNameDistinct.begin(), iVariablesNameDistinct.end() ) );
		unsigned int iVariablesCount( iVariablesNameDistinct[0] == 0 ? iVariablesNameDistinct.size() - 1 : iVariablesNameDistinct.size() );
