...
map< unsigned int, vector< long int > > QuadraticInteger::iFundamentalUnits = { { 2, vector< long int > ( {1,1} ) },
...
map< unsigned int, vector< unsigned int > > QuadraticInteger::iPellMinimalSolution = { { 2, vector< unsigned int > ( {3, 2} ) },
...
QuadraticInteger::QuadraticInteger( const int& iVal )
: a( iVal ), b( 0 )
{
...
QuadraticInteger::QuadraticInteger(int a, int b)
: a( a ), b( b )
{
...
std::map< QuadraticInteger, unsigned int > QuadraticInteger::qiPrimeDecomposition() const
{
	...
	vector< unsigned long > iDecom( iPrimeFactors<unsigned long>( abs( b ? iNorm() : a ) ) );
	unsigned int iPower;
...
vector< QuadraticInteger > QuadraticInteger::qiFactorsRationalPrime( const unsigned int& iPrime, bool bWithMultiplicities )
{
	// TODO: mettre en cache?
	if( !d )
	...
	if( iPrime % 2 && d % iPrime )
	{
		...
		if( bIsOneMod4 )
		{
			...
			vector< QuadraticInteger > qiFactors( 2, QuadraticInteger( ( iCoeffs[0] - iCoeffs[1])/2, iCoeffs[1] ) );
		...
		else
		{
			...
			vector< QuadraticInteger > qiFactors( 2, QuadraticInteger( iCoeffs[0], iCoeffs[1] ) );
			qiFactors[1].conjugate();
	...
	else if( d % iPrime == 0 ) // p divides d
	{
		if( bIsOneMod4 )
		{
			...
			return vector< QuadraticInteger >( (int)bWithMultiplicities+1, QuadraticInteger( ( iCoeffs[0] - iCoeffs[1])/2, iCoeffs[1] ) );
		}
		else
		{
			array< long int, 2 > iCoeffs( iPellEquation( iPrime ) );
			
			return vector< QuadraticInteger >( (int)bWithMultiplicities+1, QuadraticInteger( iCoeffs[0], iCoeffs[1] ) );
		}
	}
...
bool QuadraticInteger::bIsDivisbleBy( const AlgebraicInteger* ai ) const
{
	...
	if( bIsOneMod4 )
	{
		if( ( a * ( 2 * qi->a + qi->b ) + b * qi->a - b * qi->b * ( d - 1 ) / 2 ) % iNorm2 )
			return false;
	}
	else
	{
...
bool QuadraticInteger::bIsSquareOfIvertible() const
{
	...
	{
		QuadraticInteger qiUnit( QuadraticInteger::iFundamentalUnits[5][0], QuadraticInteger::iFundamentalUnits[5][1] );
		QuadraticInteger qiTemp( *this );
...
{
	return ( bIsOneMod4 ? ( a * (a+b) - b * b * ( d - 1 ) / 4 ) : ( a * a - b * b * (long int)d ) );
}
...
long int QuadraticInteger::iSQRTsup_quotient(const QuadraticInteger& qiNum, const QuadraticInteger& qiDen)
{
	long int x, y, z, s;
	...
	if( y >= 0 )
	{
		s =  iSQRTsupQuotient<unsigned long int>( x + iSQRTsup<unsigned long int>( y * y * d ), z );
...
long int QuadraticInteger::iSQRT_quotient(const QuadraticInteger& qiNum, const QuadraticInteger& qiDen)
{
	long int x, y, z, s;
	long int iNumTrace( qiNum.iTrace() ), iDenTrace( qiDen.iTrace() );
	...
	if( y >= 0 )
	{
		s = iSQRTQuotient<unsigned long int>( x + iSQRT( (unsigned long int) y * y * d ), z );
...
std::string QuadraticInteger::to_string( const string& strFormat, const bool& bProtect ) const
{
	if( strFormat == "latex" )
	{
		if( bIsOneMod4 )
		{
			if( a == 0 )
			{
				if( b == 0 )
					...
					return ( abs(b) == 1 ? ( b == 1 ? "" : "-" ) : std::to_string( b ) + "\\cdot" ) + "\\frac{1+\\sqrt{" + std::to_string( d ) + "}}{2}";
			}
			else
			{
				if( b == 0 )
					...
					return std::to_string( a ) + ( b < 0 ? "-" : "+" ) + ( abs(b) == 1 ? "" : std::to_string( abs( b ) ) + "\\cdot" ) + "\\frac{1+\\sqrt{" + std::to_string( d ) + "}}{2}";
			}
		}
		else
		{
			if( a == 0 )
			{
				if( b == 0 )
					...
					return ( abs(b) == 1 ? ( b == 1 ? "" : "-" ) : std::to_string( b ) + "\\cdot" ) + "\\sqrt{" + std::to_string( d ) + "}";
			}
			else
			{
				if( b == 0 )
					...
					return std::to_string( a ) + ( b < 0 ? "-" : "+" ) + ( abs( b ) == 1 ? "" : std::to_string( abs( b ) ) + "\\cdot" ) + "\\sqrt{" + std::to_string( d ) + "}";
			}
		}
	}
		...
		if( bIsOneMod4 )
		{
			...
			else
			{
				if( b == 0 )
					...
					return ( bProtect ? "(" : "" ) + std::to_string( a ) + ( b < 0 ? "-" : "+" ) + std::to_string( abs( b ) ) + "*(1+Sqrt[" + std::to_string( d ) + ( bProtect ? "])/2)" : "])/2" );
			}
		}
		else
		{
			...
			else
			{
				if( b == 0 )
					...
					return ( bProtect ? "(" : "" ) + std::to_string( a ) + ( b < 0 ? "-" : "+" ) + std::to_string( abs( b ) ) + "*Sqrt[" + std::to_string( d ) + ( bProtect ? "])" : "]" );
			}
		}
	}
	...
	else if( strFormat == "filename" )
	{
		...
		else
		{
			if( b == 0 )
				...
				return ( bProtect ? "(" : "" ) + std::to_string( a ) + ( b < 0 ? "-" : "+" ) + ( abs( b ) == 1 ? "" : std::to_string( abs( b ) ) ) + "T" + std::to_string( d ) + ( bProtect ? ")" : "" );
		}
	}
	else
	{
		...
		else
		{
			if( b == 0 )
				...
				return ( bProtect ? "(" : "" ) + std::to_string( a ) + ( b < 0 ? "-" : "+" ) + std::to_string( abs( b ) ) + "*T" + std::to_string( d ) + ( bProtect ? ")" : "" );
		}
	}
}

string QuadraticInteger::get_classname() const
{
