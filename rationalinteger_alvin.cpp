...
RationalInteger_AlVin::RationalInteger_AlVin( const vector< int >& iQuadraticFormCoeffs, const string &strOuputMathematicalFormat, const bool& bWriteInfo, const bool& bDebug )
: AlVin( strOuputMathematicalFormat, bWriteInfo, bDebug )
{
	...
	for( auto i : iQuadraticFormCoeffs )
	{
		...
		else if( i > 0 )
		{
			...
			aiQF.insert( lower_bound( aiQF.begin() + ( bNegativeFound ? 1 : 0 ), aiQF.end(), ri ), ri );
		}
	}
	...
	// -------------------------------------------------
	// local copy of the quadratic form (computations performs faster without the class-interface RationalInteger)
	for( auto i : aiQF )
	{
	...
	bLastTwoCoefficientsQFAre1Mod4 = riQF[ iDimension - 1 ] % 4 == 1 && riQF[ iDimension ] % 4 == 1;
...
bool RationalInteger_AlVin::PreRun()
{
	return true;
}
...
{
	vector< AlgebraicInteger* > aiPossibleNorms2( { new RationalInteger( 1 ) } ); ///< Possible values for (e,e)
	...
	iPrimeNumbers = vector< unsigned int >( iPrimeNumbers.begin(), unique( iPrimeNumbers.begin(), iPrimeNumbers.end() ) );
...
void RationalInteger_AlVin::findVector( AlgebraicInteger* aiX0, AlgebraicInteger* aiNorm2 )
{
	iBilinearProducts = vector< int >( iVectorsCount_second, 0 );
	
	findVector( dynamic_cast< RationalInteger* >( aiX0 )->iVal, dynamic_cast< RationalInteger* >( aiNorm2 )->iVal );
}

bool RationalInteger_AlVin::bCandidatePreserveEvenLattice(const unsigned int& iNorm2) const
{
	...
	if( iSumCoordinates * ( riQF[0] * iVectorCurrent[0] + riQF[1] * iVectorCurrent[1] ) % 2 )
		return false;
	
	for( i = 1; i <= iDimension; i++ )
	{
		if( iSumCoordinates * ( - riQF[0] * iVectorCurrent[0] + riQF[i] * iVectorCurrent[i] ) % 2 )
			return false;
	}
...
void RationalInteger_AlVin::addVectorChild( const std::vector< AlgebraicInteger* >& aiVector )
{
	vector< int > iV;
...
int RationalInteger_AlVin::addVector_iFindWeight(AlgebraicInteger* aiNumerator, AlgebraicInteger* aiDenominator)
{
	return -2;
}

void RationalInteger_AlVin::findVector( const unsigned int& i0, const unsigned int& iNorm2, unsigned int iIndex, unsigned int iSumComp, unsigned int iGCDComponents )
{
	if( iIndex == 1 )
	{
	...
	else if( iGCDs[iIndex] && iSumComp % iGCDs[iIndex] ) // equation a * x + b * y = z has a solution only if gcd(a, b) | z
		return;
	...
	{
		if( bLastTwoCoefficientsQFAre1Mod4 && iSumComp % 4 == 3 ) // Sum of two squares cannot be 3 mod 4
			return;
	...
	unsigned int iMax( iComponentLessThan[ iIndex ] ? min( (unsigned int) iVectorCurrent[ iComponentLessThan[ iIndex ] ], iSQRT( iSumComp ) / iSQRT( riQF[ iIndex ] ) ) : iSQRT( iSumComp ) / iSQRT( riQF[ iIndex ] ) );
	...
	for( unsigned int i( iCeilQuotient( iNorm2, 2 * riQF[iIndex] ) ); i <= iMax; i++ )
	{
		if( !( 2 * i * riQF[ iIndex ] % iNorm2 ) )
		{
			for( unsigned int j( 0 ); j < iVectorsCount_second; j++ )
			{
				iBilinearProducts[j] += riQF[iIndex] * ( i - iLastCoeff ) * iVectors[j + iDimension][iIndex];
				...
				if( iBilinearProducts[j] > 0 ) // iVectorCurrent is not admissible
				{
					...
					for( unsigned int k( 0 ); k < iVectorsCount_second; k++ )
						iBilinearProducts[k] -= riQF[iIndex] * iVectors[k + iDimension][iIndex] * ( k <= j ? i : iLastCoeff );
			...
			if( iIndex == iDimension && riQF[ iIndex ] * i * i == iSumComp )
			{
				...
				for( unsigned int i( 0 ); i < iVectorsCount_second; i++ )
					iBilinearProducts[i] -= riQF[iIndex] * iLastCoeff * iVectors[i + iDimension][iIndex];
			...
			if( iIndex < iDimension )
			{
				if( riQF[ iIndex ] * i * i <= iSumComp )
					findVector( i0, iNorm2, iIndex + 1, iSumComp - riQF[ iIndex ] * i * i, ugcd( iGCDComponents, i ) );
			}
		}
	}
...
string RationalInteger_AlVin::get_strField() const
{
	return string( "Q" );
}
