...
array< array< array<mpq_class, 2>, 3 >, 2 > RCyclotomic7Integer::mpqRationalApproximations ={{ {{ {{mpq_class(mpz_class("21689544433045364711933845"), mpz_class("17393664153274833967747621")), mpq_class(mpz_class("10011913795355660647342091"), mpz_class("8028931480120743221914558")) }}, 
	{{mpq_class( mpz_class("-17393664153274833967747621"), mpz_class("39083208586320198679681466")), mpq_class( mpz_class("-6693130287087395697996053"), mpz_class("15039327240109012928088481")) }}, 
	{{mpq_class( mpz_class("-39083208586320198679681466"), mpz_class("21689544433045364711933845")), mpq_class( mpz_class("-18040845275476403869256649"), mpz_class("10011913795355660647342091")) 	}} }},
	
	{{ {{mpq_class(mpz_class("11861716548213806650665787950831323922297563629148852441750139798979225553949651060857197331644225179"), mpz_class("9512358111433361619147889679014039521375970015305751723446393067643895366950437131247228319682746513")), mpq_class(mpz_class("4279766040305502977765390182184089005892462452168265202757047478106033\
2669976203083482085854272332411"), mpz_class("34321058881370331868943003048280828283764132694076140500709644855433530765149420395818437777189360798")) }}, 
	{{mpq_class( mpz_class("-9512358111433361619147889679014039521375970015305751723446393067643895366950437131247228319682746513"), mpz_class("2137407465964716826981367762984536344367353364445460416519653286662312\
0920900088192104425651326971692")), mpq_class( mpz_class("-3600124177956973113034952537649417869741124644817474047711165575344850787496486426221461372761653920761"), mpz_class("8089405598719431309982823636977886334493376006819893508897776026292217\
217866533715958193087866151300015")) }}, 
	{{mpq_class( mpz_class("-21374074659647168269813677629845363443673533644454604165196532866623120920900088192104425651326971692"), mpz_class("1186171654821380665066578795083132392229756362914885244175013979897922\
5553949651060857197331644225179")), mpq_class( mpz_class("-77118719284425361646596904870121718342688757215758792528280119636493863435125623479300523631461693209"), mpz_class("42797660403055029777653901821840890058924624521682652027570474781060332669976203083482085854272332411")) }} }} }}	;
...
void RCyclotomic7Integer::computePrimesDecomposition_factorMinimalPolynomial( GEN& gFactors, const unsigned int& iPrime )
{
	...
	for( unsigned int i(0); i < iMinimalPolynomialSize; i++ )
	{
		...
		if( iCoefficient < 0 ) // We add some multiple of iPrime so that iCoefficient is positive
			iCoefficient += iPrime * ( (-iCoefficient % iPrime) ? -iCoefficient / iPrime + 1 : -iCoefficient / iPrime );
...
bool RCyclotomic7Integer::computePrimeDecomposition(const unsigned int& iPrime)
{
	...
	unsigned int iFactorsCount( (unsigned int)(lg(gFactors) - 1) ); // Number of irreducible factors
	// The above are not really useful since gExponents * gFactors * iFactorsCount = 3.
	
	if( iFactorsCount == 1 && gExponents[1] == 1 ) // iPrime is prime in O_K
	{
		iPrimesDecomposition[ iPrime ] = vector< RCyclotomic7Integer >( { RCyclotomic7Integer( iPrime ) } ); 
	}
	else
	{
		iPrimesDecomposition[ iPrime ] = vector< RCyclotomic7Integer >( 0 ); 
		long int iC0, iC1, iNorm, iResidualNorm;
		for( unsigned int i(1); i <= iFactorsCount && iPrimesDecomposition[ iPrime ].size() < iFactorsCount ;i++ )
		{
			...
			for( int k(-2); k <= 1; k++ )
			{
				...
				if( !mpz_norm.fits_slong_p() )
					throw( string( "RCyclotomic7Integer::computePrimeDecomposition: Number too big" ) );
				...
				vector< unsigned long > iResidualPrimeFactors( iPrimeFactors<unsigned long>( iResidualNorm ) );
...
void RCyclotomic7Integer::computePrimesDecomposition_addFactor( const unsigned int& iPrime, const unsigned int& iFactorsCount, RCyclotomic7Integer rci )
{
	...
	for( unsigned int j(0); j < 3; j++ )
	{
		...
		{
			if( (*ptrList)[i].bIsAssociateTo( &rci ) )
				break;
		}
...
RCyclotomic7Integer::RCyclotomic7Integer( const array< mpz_class, 3 >& iCoefficients )
: iC( iCoefficients )
{	
}
...
std::string RCyclotomic7Integer::to_string(const std::string& strFormat, const bool& bProtect) const
{
	...
	else
	{
		if( strFormat == "mathematica" )
			return string( "RCI7[{" + iC[0].get_str() + "," + iC[1].get_str() + "," + iC[2].get_str() + "}]" );
...
interval RCyclotomic7Integer::to_interval() const
{
	...
	return ( interval( gaol_intervalFromMPZclass( iC[0] ) ) * gaol_l1 + interval( gaol_intervalFromMPZclass( iC[1] ) ) * gaol_l2 + interval( gaol_intervalFromMPZclass( iC[2] ) ) * gaol_l3 );
}
...
{
	return ( iC[0] * iC[0] * iC[0] + iC[1] * iC[1] * iC[1] + iC[2] * iC[2] * iC[2] - iC[0] * iC[1] * iC[2] + 3*(iC[0] * iC[0] * iC[1] + iC[1] * iC[1] * iC[2] + iC[0] * iC[2] * iC[2]) - 4 * ( iC[0] * iC[1] * iC[1] + iC[0] * iC[0] * iC[2] + iC[1] * iC[2] * iC[2]) );
}

bool RCyclotomic7Integer::bIsLongInt() const
{
	if( iC[0] == iC[1] && iC[1] == iC[2] )
...
bool RCyclotomic7Integer::bIsGreaterOEThan(const int& n) const
{
	...
	interval i( interval( gaol_intervalFromMPZclass( iC[0] ) ) * gaol_l1 + interval( gaol_intervalFromMPZclass( iC[1] ) ) * gaol_l2 + interval( gaol_intervalFromMPZclass( iC[2] ) ) * gaol_l3 - interval( n ) );
...
bool RCyclotomic7Integer::bIsLessOEThan(const AlgebraicInteger& ai) const
{
	...
	interval i( interval( gaol_intervalFromMPZclass( iC[0] - rci.iC[0] ) ) * gaol_l1 + interval( gaol_intervalFromMPZclass( iC[1] - rci.iC[1] ) ) * gaol_l2 + interval( gaol_intervalFromMPZclass( iC[2] - rci.iC[2] ) ) * gaol_l3 );
	...
	mpq_class mpqInf( ( i0 > 0 ? mpqRationalApproximations[0][0][0] : mpqRationalApproximations[0][0][1] ) * i0 );
	mpqInf += ( i1 > 0 ? mpqRationalApproximations[0][1][0] : mpqRationalApproximations[0][1][1] ) * i1;
	mpqInf += ( i2 > 0 ? mpqRationalApproximations[0][2][0] : mpqRationalApproximations[0][2][1] ) * i2;
	
	if( mpqInf > 0 )
		return false;
	
	mpq_class mpqSup( ( i0 > 0 ? mpqRationalApproximations[0][0][1] : mpqRationalApproximations[0][0][0] ) * i0 );
	mpqSup += ( i1 > 0 ? mpqRationalApproximations[0][1][1] : mpqRationalApproximations[0][1][0] ) * i1;
	mpqSup += ( i2 > 0 ? mpqRationalApproximations[0][2][1] : mpqRationalApproximations[0][2][0] ) * i2;
	...
	mpqInf = ( i0 > 0 ? mpqRationalApproximations[1][0][0] : mpqRationalApproximations[1][0][1] ) * i0;
	mpqInf += ( i1 > 0 ? mpqRationalApproximations[1][1][0] : mpqRationalApproximations[1][1][1] ) * i1;
	mpqInf += ( i2 > 0 ? mpqRationalApproximations[1][2][0] : mpqRationalApproximations[1][2][1] ) * i2;
	
	if( mpqInf > 0 )
		return false;
	
	mpqSup = ( i0 > 0 ? mpqRationalApproximations[1][0][1] : mpqRationalApproximations[1][0][0] ) * i0;
	mpqSup += ( i1 > 0 ? mpqRationalApproximations[1][1][1] : mpqRationalApproximations[1][1][0] ) * i1;
	mpqSup += ( i2 > 0 ? mpqRationalApproximations[1][2][1] : mpqRationalApproximations[1][2][0] ) * i2;
	
	if( mpqSup < 0 )
		return true;
	
	throw( string( "RCyclotomic7Integer::bIsLessOEThan : Cannot decide" ) );
}
...
bool RCyclotomic7Integer::bIsLessThan(const int& n) const
{
	...
	throw( string( "RCyclotomic7Integer::bIsLessThan(int) : Cannot decide" ) );
}
...
bool RCyclotomic7Integer::divideByIfDivisible(const AlgebraicInteger* ai)
{
	...
	if( ( (- a0*b0*b0 - a1*b0*b0 + a2*b0*b0 - 2*a0*b0*b1 + 3*a1*b0*b1 - a2*b0*b1 + a0*b1*b1 - a1*b1*b1 + 3*a0*b0*b2 - a2*b0*b2 + 2*a0*b1*b2 - 3*a1*b1*b2 + 2*a2*b1*b2 - 2*a0*b2*b2 + 2*a1*b2*b2 - a2*b2*b2) % iNormb ) != 0 || ( (- a0*b0*b0 - 2*a1*b0*b0 + 2*a2*b0*b0 - a0*b0*b1 + 3*a1*b0*b1 + a0*b1*b1 - a1*b1*b1 - a2*b1*b1 + 2*a0*b0*b2 + 2*a1*b0*b2 - 3*a2*b0*b2 - a0*b1*b2 - 2*a1*b1*b2 + 3*a2*b1*b2 + a1*b2*b2 - a2*b2*b2) % iNormb ) != 0 || ( (- a0*b0*b0 + a2*b0*b0 - 3*a0*b0*b1 + 2*a1*b0*b1 + 2*a2*b0*b1 + 2*a0*b1*b1 - a1*b1*b1 - 2*a2*b1*b1 + 3*a0*b0*b2 - a1*b0*b2 - 2*a2*b0*b2 - a1*b1*b2 + 3*a2*b1*b2 - a0*b2*b2 + a1*b2*b2 - a2*b2*b2) % iNormb ) != 0 )
		return false;
	
	iC[0] = (- a0*b0*b0 - a1*b0*b0 + a2*b0*b0 - 2*a0*b0*b1 + 3*a1*b0*b1 - a2*b0*b1 + a0*b1*b1 - a1*b1*b1 + 3*a0*b0*b2 - a2*b0*b2 + 2*a0*b1*b2 - 3*a1*b1*b2 + 2*a2*b1*b2 - 2*a0*b2*b2 + 2*a1*b2*b2 - a2*b2*b2) / iNormb;
	
	iC[1] = (- a0*b0*b0 - 2*a1*b0*b0 + 2*a2*b0*b0 - a0*b0*b1 + 3*a1*b0*b1 + a0*b1*b1 - a1*b1*b1 - a2*b1*b1 + 2*a0*b0*b2 + 2*a1*b0*b2 - 3*a2*b0*b2 - a0*b1*b2 - 2*a1*b1*b2 + 3*a2*b1*b2 + a1*b2*b2 - a2*b2*b2) / iNormb;
	
	iC[2] = (- a0*b0*b0 + a2*b0*b0 - 3*a0*b0*b1 + 2*a1*b0*b1 + 2*a2*b0*b1 + 2*a0*b1*b1 - a1*b1*b1 - 2*a2*b1*b1 + 3*a0*b0*b2 - a1*b0*b2 - 2*a2*b0*b2 - a1*b1*b2 + 3*a2*b1*b2 - a0*b2*b2 + a1*b2*b2 - a2*b2*b2) / iNormb;
...
bool RCyclotomic7Integer::bIsAssociateTo(const AlgebraicInteger* ai)
{
	const RCyclotomic7Integer* rci( dynamic_cast< const RCyclotomic7Integer* >( ai ) );
	mpz_class iNormb( rci->iNorm() );
	...
	return ( ( (- a0*b0*b0 - a1*b0*b0 + a2*b0*b0 - 2*a0*b0*b1 + 3*a1*b0*b1 - a2*b0*b1 + a0*b1*b1 - a1*b1*b1 + 3*a0*b0*b2 - a2*b0*b2 + 2*a0*b1*b2 - 3*a1*b1*b2 + 2*a2*b1*b2 - 2*a0*b2*b2 + 2*a1*b2*b2 - a2*b2*b2) % iNormb ) == 0 && ( (- a0*b0*b0 - 2*a1*b0*b0 + 2*a2*b0*b0 - a0*b0*b1 + 3*a1*b0*b1 + a0*b1*b1 - a1*b1*b1 - a2*b1*b1 + 2*a0*b0*b2 + 2*a1*b0*b2 - 3*a2*b0*b2 - a0*b1*b2 - 2*a1*b1*b2 + 3*a2*b1*b2 + a1*b2*b2 - a2*b2*b2) % iNormb ) == 0 && ( (- a0*b0*b0 + a2*b0*b0 - 3*a0*b0*b1 + 2*a1*b0*b1 + 2*a2*b0*b1 + 2*a0*b1*b1 - a1*b1*b1 - 2*a2*b1*b1 + 3*a0*b0*b2 - a1*b0*b2 - 2*a2*b0*b2 - a1*b1*b2 + 3*a2*b1*b2 - a0*b2*b2 + a1*b2*b2 - a2*b2*b2) % iNormb ) == 0 );
}
...
{	
	...
	for( auto iNPF : iNormPrimesFactors )
	{
		...
		{
			while( rci1.divideByIfDivisible( &rciFactor ) && rci2.divideByIfDivisible( &rciFactor ) )
...
vector< RCyclotomic7Integer > RCyclotomic7Integer::rciPrimeFactors() const
{
	...
	{	
		...
		{
			if( bIsDivisbleBy( &rciF ) ) // rciF divides either *this or the conjugate of *this
...
map< RCyclotomic7Integer, unsigned int > RCyclotomic7Integer::rciPrimeDecomposition() const
{
	if( ( iC[0] == 0 && iC[1] == 0 && iC[2] == 0 ) || bIsInvertible() )
	...
	mpz_class mpz_Norm( iNorm() );
	if( !mpz_Norm.fits_slong_p() )
		throw( string( "RCyclotomic7Integer: rciPrimeDecomposition: Component too big" ) );
	...
	for( auto iNPF : iNormPrimesFactors )
	{
		...
		{
			iPower = 0;
			while( rciTemp.divideByIfDivisible( &rciF ) ) // rciF divides either *this or the conjugate of *this
				iPower++;
...
RCyclotomic7Integer& RCyclotomic7Integer::operator=(const RCyclotomic7Integer& rci)
{
	iC[0] = rci.iC[0];
	iC[1] = rci.iC[1];
	iC[2] = rci.iC[2];
...
RCyclotomic7Integer RCyclotomic7Integer::operator*( const RCyclotomic7Integer& rci ) const
{
...
RCyclotomic7Integer RCyclotomic7Integer::operator+( const RCyclotomic7Integer& rci ) const
{
	return RCyclotomic7Integer( { iC[0] + rci.iC[0], iC[1] + rci.iC[1], iC[2] + rci.iC[2] } );
}
...
{
	map< RCyclotomic7Integer, unsigned int > rciDecomp( this->rciPrimeDecomposition() );
	unsigned int i;
