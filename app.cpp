...
void App::readMainParameters( int argc, char **argv )
{
	...
	if( regexp.preg_match_all( "(-k|-field|k=|field=)[[:space:]Q]*[\\[\\(]?[[:space:]]*sqrt[[:space:]\\[\\(]*([0-9]{1,5})", strParams, regexpRes, PCRE_CASELESS ) )
	{
		...
		if( !QuadraticInteger::bIsDAdmissible( iFieldSupp ) )
			throw( string( "Field of definition: quadratic field: the following value is not admissible: " + regexpRes[2][0] ) );
	...
	if( regexp.preg_match_all( "-minv[[:space:]]+([[:digit:]]{1,4})", strParams, regexpRes ) )
	{
	...
	if( regexp.preg_match_all( "-maxv[[:space:]]+([[:digit:]]{1,4})", strParams, regexpRes ) )
	{
		str_replace( strParams, regexpRes[0][0], "" );
	...
	// --------------------------------------------------
	// Output format
	if( regexp.preg_match_all( "-oformat[[:space:]=]?(mathematica|generic|latex|pari)", strParams, regexpRes, PCRE_CASELESS ) )
	{
	...
	else if( strField == "quadratic" )
	{
		...
		{	
			...
			{	
				...
				else if( regexp.preg_match_all( "([+-]{0,1})([[:digit:]]+)([+-]{1,1})([[:digit:]]*)[\\*]*T", c, regexpRes ) )
				{
					string strN1( regexpRes[1][0] + regexpRes[2][0] ), strN2( regexpRes[3][0] );
					
					strN2 += regexpRes[4][0] == "" ? "1" : regexpRes[4][0];
					
					aiQF.push_back( new QuadraticInteger( stoi( strN1 ), stoi( strN2 ) ) );
				}
				else if( regexp.preg_match_all( "([+-]{0,1})([[:digit:]]*)[\\*]*T([+-]{1,1})([[:digit:]]*)", c, regexpRes ) )
				{
				...
				else if( regexp.preg_match_all( "^([-[:digit:]]+)[*]?(T|t)$", c, regexpRes ) ) // non-integer part only
				{
					if( regexpRes[1][0] == "-" )
						regexpRes[1][0] = "-1";
	...
	else if( strField == "rc7" )
	{
		if( regexp.preg_match_all( "(-f|-qf)([[:digit:],\\+\\-\\* \\[\\]\\(\\)]+)", strParams, regexpRes ) )
		{
			...
			if( ( iRegexpCount = regexp2.preg_match_all( "([[:digit:]\\*\\-]*)\\[([[:digit:];\\-]+)\\]", regexpRes[0][0], regexpRes2 ) ) > 0 )
			{
				for( int i(0); i < iRegexpCount; i++ )
				{
					...
					aiQF.push_back( new RCyclotomic7Integer( {iCoefficient * ( strCoeffs[0] == "" ? 0 : stoi( strCoeffs[0] ) ), iCoefficient * ( strCoeffs[1] == "" ? 0 : stoi( strCoeffs[1] ) ), iCoefficient * ( strCoeffs[2] == "" ? 0 : stoi( strCoeffs[2] ) )} ) );
...
AlVin* App::instanciateAlVin()
{
	...
	if( strField == "rationals" )
	{
		...
		v = new RationalInteger_AlVin( iQF, strOuputMathematicalFormat, true, bDebug );
	}
	else if( strField == "quadratic" )
	{
		...
		v = new QuadraticInteger_AlVin( qiQF, strOuputMathematicalFormat, true, bDebug );
	}
	#ifdef _RC7AVAILABLE_
	else if( strField == "rc7" )
	{
		...
		v = new RCyclotomic7Integer_AlVin( rciQF, strOuputMathematicalFormat, true, bDebug );
	}
	#endif
...
InfiniteNSymetries* App::instanciateInfiniteNSymetries(AlVin* v)
{
	...
	if( iMaxVectors < v->get_iDimension() )
		throw( string( "The number of vectors should be at least equal to the dimension" ) );
...
void App::Run()
{
	...
	try{
			...
			if( bCheckNR )
			{
				try{
								...
								if( strOuputMathematicalFormat == "latex" )
								{
									for( unsigned int i(0); i < iSize; i++ )
									{
										if( inv.iPermutation[i] == i )
											cout << ( i ? ", " : "\t\t" ) << "e_" << ( inv.iVertices[i] < 9 ? "" : "{" ) << ( inv.iVertices[i] + 1 ) << ( inv.iVertices[i] < 9 ? "" : "}" );
										else if( i < inv.iPermutation[i] )
											cout << ( i ? ", " : "\t\t" ) << "e_" << ( inv.iVertices[i] < 9 ? "" : "{" ) << ( inv.iVertices[i] + 1 ) << ( inv.iVertices[i] < 9 ? "" : "}" ) << " \\leftrightarrow e_" << ( inv.iVertices[inv.iPermutation[i]] < 9 ? "" : "{" ) << ( inv.iVertices[inv.iPermutation[i]] + 1 ) << ( inv.iVertices[inv.iPermutation[i]] < 9 ? "" : "}" );
									}
								}
								else
								{
									...
									{
										if( inv.iPermutation[i] == i )
											cout << ( i ? ", " : "\t\t" ) << "e" << ( inv.iVertices[i] + 1 );
										else if( i < inv.iPermutation[i] )
											cout << ( i ? ", " : "\t\t" ) << "e" << ( inv.iVertices[i] + 1 ) << " <-> e" << ( inv.iVertices[inv.iPermutation[i]] + 1 );
									}
								}
								cout << endl;
							}
					...
					else
					{
						...
						if( bDebug )
						{
							cout << "\tFor the found involutions, a basis of the fixed points space is:" << endl;
		...
		cout << "\nComputation time: " << chrono::duration<double, milli>(chrono::system_clock::now()-timeStart).count() / 1000 << "s" << endl;
