...
void RationalInteger_NotReflexive::createSystemEquations( NotReflexive_Graph nrg )
{
	...
	{
		// The cristallographic condition implies that the coefficient x_i is divisible by (e,e) / gcd( (e,e), 2 * alpha_i )
		// Hence, we will do a substitution.
		...
		for( unsigned int i(0); i < iVariablesCount; i++ )
		{
			...
			strEquation += ( i ? " + " : "-" ) + aiTemp->to_string() + " * x" + to_string(i + 1) + "^2"; 
			
			if( nrg.iVariablesGreaterThan[i] )
				strInequalities += " && x" + to_string( i ) + " >= x" + to_string( nrg.iVariablesGreaterThan[i] );
		...
		for( vector< short unsigned int >::const_iterator it( upper_bound( nrg.iGraphVertices.begin(), nrg.iGraphVertices.end(), iDimension - 1 ) ); it != nrg.iGraphVertices.end(); ++it )
		{
			...
			strEquation += " && ";
			for( unsigned int i(0); i < iVariablesCount; i++ )
				strEquation += ( i ? " + " : "-" ) + aiNumberVariablesCount[i]->to_string( strOFormat, true ) + "*x" + to_string(i + 1); 
			strEquation += " == 0";
		...
		strGlobalEquation += ( strGlobalEquation == "" ? "" : " || " ) + string( "(") + strEquation + ")"; 
