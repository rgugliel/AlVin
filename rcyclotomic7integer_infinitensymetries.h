...
namespace Eigen 
{
	...
	template<> struct NumTraits<Rational<RCyclotomic7Integer>> ///< \struct NumTraits<Rational<RCyclotomic7Integer>> rcyclotomic7integer_infinitensymetries.h "Specialization for Eigen matrices to RCyclotomic7Integer type"
	{
	...
	namespace internal {
		template<>
		struct significant_decimals_impl<Rational<RCyclotomic7Integer>> ///< \struct significant_decimals_impl<Rational<RCyclotomic7Integer>> rcyclotomic7integer_infinitensymetries.h "Specialization for Eigen matrices to RCyclotomic7Integer type"
		{
...
{
	private:
		vector< Matrix< Rational<RCyclotomic7Integer>, Dynamic, 1 > > rciVectorsC; ///< Vectors found by AlVin, in column
		vector< vector< Rational<RCyclotomic7Integer> > > qiDottedWeights; ///< Weights of the dotted lines
		
		Matrix< Rational<RCyclotomic7Integer>, Dynamic, Dynamic > rciBasisFixedPoints; ///< Basis of the fixed points of the involutions, in column
		
		vector< Rational<RCyclotomic7Integer> > rciQF; ///< Coefficients of the diagonal quadratic form (in fractions)
		vector< vector< RCyclotomic7Integer > > riVectorsProducts; ///< Products of each pair of vecteurs
	...
	public:
		...
		Rational<RCyclotomic7Integer> rciVectorNorm( const Matrix< Rational<RCyclotomic7Integer>, Dynamic, 1 >& rciV ) const;
		Rational<RCyclotomic7Integer> rciVectorsProduct( const Matrix< Rational<RCyclotomic7Integer>, Dynamic, 1 >& rciV1, const Matrix< Rational<RCyclotomic7Integer>, Dynamic, 1 >& rciV2 ) const;
