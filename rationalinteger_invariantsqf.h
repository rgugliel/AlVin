 ...
 * 
 * \class InvariantsQF
 * \brief Computation of the commensurability of a quadratic form over the rationals
*/
 ...
 * 
 * \class InvariantsQF
 * \brief Compute the invariants of an integral quadratic form of signature (n,1)
*/
...
class InvariantsQF
{
	...
	private:
		 ...
		 * 	Find the ramification of an elementary quaternion algebra with coefficients in Z
		 * 
		 * 	\param a( int ) -1 or a prime number or -prime
		 * 	\param b( int ) -1 or a prime number or -prime
		 */
		vector< unsigned int > iRamificationElementaryQuaternionAlgebra( int a, int b ) const;
		
		vector< unsigned int > iRamificationProduct( const vector< unsigned int >& iR1, const vector< unsigned int >& iR2 );
};
