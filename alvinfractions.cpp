...
: aiPossibleNorms2( aiPossibleNorms2 ), aiPossibleNorms2_max( aiPossibleNorms2[ aiPossibleNorms2.size() - 1] ), iLastMaximum( 0 ), iBatchSize( 1 )
{
}
...
vector< AlVinFraction* > AlVinFractions::getNextAlVinFraction()
{
	...
	if( alvinfractions.size() && alvinfractions_it == alvinfractions.end() ) // All the fractions were tested
	{
		for( auto i : alvinfractions )
			delete i;
	...
	}while( alvinfractions_it != alvinfractions.end() && aiNumerator->bIsEqualTo( *(*alvinfractions_it)->aiNumerator ) );
...
const std::vector< AlgebraicInteger* >* AlVinFractions::get_ptraiPossibleNorm2() const
{
	return &aiPossibleNorms2;
}
