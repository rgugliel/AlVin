...
map< unsigned int, vector< long int > > QuadraticIntegerBig::iFundamentalUnits = { { 2, vector< long int > ( {1,1} ) },
...
map< unsigned int, vector< unsigned int > > QuadraticIntegerBig::iPellMinimalSolution = { { 2, vector< unsigned int > ( {3, 2} ) },
...
QuadraticIntegerBig::QuadraticIntegerBig( const int& iVal )
: a( iVal ), b( 0 )
{
...
vector< QuadraticIntegerBig > QuadraticIntegerBig::qiPrimeFactors() const
{
	...
	auto iNormPrimesFactors( iPrimeFactors( iN.get_ui() ) );
	...
	{	
		...
		for( auto qiF : qiTemp )
		{
			if( bIsDivisbleBy( &qiF ) ) // qiF divides either *this or the conjugate of *this
...
array< long int, 2 > QuadraticIntegerBig::iPellEquation(const unsigned int& iN )
{
	...
	for( unsigned int iNegative( 0 ); iNegative <= 1; iNegative++ )
	{
		...
		{
			L1 = 0;
			L2 = iSQRT( ( ( iN * ( iPellMinimalSolution[ d ][0] - 1 ) ) % ( 2 * d ) ) ? ( iN * ( iPellMinimalSolution[ d ][0] - 1 ) ) / ( 2 * d ) + 1 : ( iN * ( iPellMinimalSolution[ d ][0] - 1 ) ) / ( 2 * d ) );
		}
		else
		{
			L1 = iSQRT( iN / d );
		...
		for( unsigned int y( L1 ); y <= L2; y++ )
		{
			...
			if( iXTemp >= 0 ) // if it is a square
			{
				...
				if( iSqrt * iSqrt == iXTemp && ( !bIsOneMod4 || ( ( iXTemp%2 ) == (y%2) ) ) ) // if iXTemp is a square and if the x,y have the same parity if d=1(4)
					return array< long int, 2 >( { iSqrt, y } );
			}
		}
	}
	
	throw( string( "Pell equation was not solved for d=" + std::to_string( d ) + " and n=" + std::to_string( iN ) ) );
}

std::map< QuadraticIntegerBig, unsigned int > QuadraticIntegerBig::qiPrimeDecomposition() const
{
	...
	map< unsigned long, unsigned int > iDecom( iPrimeDecomposition<long unsigned>( iN.get_ui() ) );
	unsigned int iPower;
...
vector< QuadraticIntegerBig > QuadraticIntegerBig::qiFactorsRationalPrime( const unsigned int& iPrime, bool bWithMultiplicities )
{
	// TODO: mettre en cache?
	if( !d )
	...
	if( iPrime % 2 && d % iPrime )
	{
		...
		if( bIsOneMod4 )
		{
			array< long int, 2 > iCoeffs( QuadraticIntegerBig::iPellEquation( 4 * iPrime ) );
				
			vector< QuadraticIntegerBig > qiFactors( 2, QuadraticIntegerBig( ( iCoeffs[0] - iCoeffs[1])/2, iCoeffs[1] ) );
		...
		else
		{
			...
			vector< QuadraticIntegerBig > qiFactors( 2, QuadraticIntegerBig( iCoeffs[0], iCoeffs[1] ) );
			qiFactors[1].conjugate();
	...
	else if( d % iPrime == 0 ) // p divides d
	{
		if( bIsOneMod4 )
		{
			...
			return vector< QuadraticIntegerBig >( (int)bWithMultiplicities+1, QuadraticIntegerBig( ( iCoeffs[0] - iCoeffs[1])/2, iCoeffs[1] ) );
		}
		else
		{
			array< long int, 2 > iCoeffs( QuadraticIntegerBig::iPellEquation( iPrime ) );
			
			return vector< QuadraticIntegerBig >( (int)bWithMultiplicities+1, QuadraticIntegerBig( iCoeffs[0], iCoeffs[1] ) );
		}
	}
	...
	throw( string( "qiFactorsRationalPrime: unknown case (d=" + std::to_string( d ) + ", p=" + std::to_string( iPrime ) + ")" ) );
}
...
bool QuadraticIntegerBig::bIsEqualTo(const int& n) const
{
	return ( a == n && b == 0 );
}

bool QuadraticIntegerBig::bIsEqualTo(const AlgebraicInteger& ai) const
{
...
bool QuadraticIntegerBig::bIsGreaterThan(const int& n) const
{
...
bool QuadraticIntegerBig::bIsGreaterOEThan(const int& n) const
{
...
bool QuadraticIntegerBig::bIsLessThan(const AlgebraicInteger& ai) const
{
	mpz_class x( dynamic_cast< const QuadraticIntegerBig& >( ai ).a );
	mpz_class y( dynamic_cast< const QuadraticIntegerBig& >( ai ).b );
	
	mpz_class aTemp( bIsOneMod4 ? ( mpz_class(a-x)*2 + b-y ) : mpz_class(a-x) );
	mpz_class bTemp( b - y );
...
bool QuadraticIntegerBig::bIsLessOEThan(const AlgebraicInteger& ai) const
{
	mpz_class x( dynamic_cast< const QuadraticIntegerBig& >( ai ).a );
	mpz_class y( dynamic_cast< const QuadraticIntegerBig& >( ai ).b );
...
bool QuadraticIntegerBig::bIsLessThan(const int& n) const
{
...
bool QuadraticIntegerBig::bIsDivisbleBy( const AlgebraicInteger* ai ) const
{
	const QuadraticIntegerBig* qi( dynamic_cast< const QuadraticIntegerBig* >( ai ) );
	...
	if( bIsOneMod4 )
	{
		if( ( a * ( 2 * qi->a + qi->b ) + b * qi->a - b * qi->b * ( d - 1 ) / 2 ) % iNorm2 != 0 )
			return false;
	}
	else
	{
...
void QuadraticIntegerBig::gcd(const AlgebraicInteger* ai)
{
	...
	{
		while( qi1.divideByIfDivisible( &qiFactor ) && qi2.divideByIfDivisible( &qiFactor ) )
			this->multiplyBy( &qiFactor );
	}
}

bool QuadraticIntegerBig::bIsSquareOfIvertible() const
{
	if( this->bIsInvertible() )
	{
		QuadraticIntegerBig qiUnit( QuadraticIntegerBig::iFundamentalUnits[5][0], QuadraticIntegerBig::iFundamentalUnits[5][1] );
		QuadraticIntegerBig qiTemp( *this );
...
bool QuadraticIntegerBig::bIsInvertible() const
{
...
mpz_class QuadraticIntegerBig::iSQRTsup_quotient(const QuadraticIntegerBig& qiNum, const QuadraticIntegerBig& qiDen)
{
	mpz_class x, y, z, s;
	...
	if( y >= 0 )
	{
		...
		s =  iSQRTsupQuotient( x + is, z );
		
		while( ( z * (s+1) * (s+1) - x )*( z * (s+1) * (s+1) - x ) <= y * y * d )
			s--;
	}
	else
	{
...
mpz_class QuadraticIntegerBig::iSQRT_quotient(const QuadraticIntegerBig& qiNum, const QuadraticIntegerBig& qiDen)
{
	mpz_class x, y, z, s;
	mpz_class iNumTrace( qiNum.iTrace() ), iDenTrace( qiDen.iTrace() );
...
std::string QuadraticIntegerBig::to_string( const string& strFormat, const bool& bProtect ) const
{
		...
		if( bIsOneMod4 )
		{
			...
			else
			{
				if( b == 0 )
					...
					return ( bProtect ? "(" : "" ) + a.get_str() + ( b < 0 ? "-" : "+" ) + babs.get_str() + "*(1+Sqrt[" + std::to_string( d ) + ( bProtect ? "])/2)" : "])/2" );
			}
		}
		else
		{
			...
			else
			{
				if( b == 0 )
					...
					return ( bProtect ? "(" : "" ) + a.get_str() + ( b < 0 ? "-" : "+" ) + babs.get_str() + "*Sqrt[" + std::to_string( d ) + ( bProtect ? "])" : "]" );
			}
		}
	...
	else if( strFormat == "filename" )
	{
		...
		else
		{
			if( b == 0 )
				...
				return ( bProtect ? "(" : "" ) + a.get_str() + ( b < 0 ? "-" : "+" ) + ( abs( b ) == 1 ? "" : babs.get_str() ) + "T" + std::to_string( d ) + ( bProtect ? ")" : "" );
		}
	}
	else
	{
		...
		else
		{
			if( b == 0 )
				...
				return ( bProtect ? "(" : "" ) + a.get_str() + ( b < 0 ? "-" : "+" ) + babs.get_str() + "*T" + std::to_string( d ) + ( bProtect ? ")" : "" );
		}
	}
}
...
// ------------------------------------------------------------------------
// Operators
QuadraticIntegerBig& QuadraticIntegerBig::operator=( const QuadraticIntegerBig& qi)
{
	a = qi.a;
	b = qi.b;
...
QuadraticIntegerBig QuadraticIntegerBig::operator+( const QuadraticIntegerBig& qi ) const
{
