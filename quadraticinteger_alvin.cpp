...
QuadraticInteger_AlVin::QuadraticInteger_AlVin( const vector< QuadraticInteger >& iQuadraticFormCoeffs, const string &strOuputMathematicalFormat, const bool& bWriteInfo, const bool& bDebug )
: AlVin( strOuputMathematicalFormat, bWriteInfo, bDebug ), d( QuadraticInteger::d ), dISqrt( iSQRT( (unsigned int) QuadraticInteger::d ) ), bIsOneMod4( QuadraticInteger::bIsOneMod4 )
{
	...
	for( auto i : iQuadraticFormCoeffs )
	{
		...
		else if( i.bIsGreaterThan( 0 ) )
		{
			...
			aiQF.insert( lower_bound( aiQF.begin() + ( bNegativeFound ? 1 : 0 ), aiQF.end(), qi ), qi );
		}
	}
...
{
	vector< AlgebraicInteger* > aiPossibleNorms2( { new QuadraticInteger( 1 ) } ); ///< Possible values for (e,e)
	...
	qiFactors = vector< QuadraticInteger >( qiFactors.begin(), unique( qiFactors.begin(), qiFactors.end() ) );
	
	// Prime factors of 2
	vector< QuadraticInteger > qiTemp( QuadraticInteger::qiFactorsRationalPrime( 2, true ) ); // here we want the two factors of two (in case it splits)
	qiFactors.insert( qiFactors.end(), qiTemp.begin(), qiTemp.end() );
	
	// Fundamental unit
	qiFactors.push_back( QuadraticInteger( QuadraticInteger::iFundamentalUnits[d][0], QuadraticInteger::iFundamentalUnits[d][1] ) );
...
void QuadraticInteger_AlVin::addVectorChild(const std::vector< AlgebraicInteger* >& aiVector)
{
...
int QuadraticInteger_AlVin::addVector_iFindWeight(AlgebraicInteger* aiNumerator, AlgebraicInteger* aiDenominator)
{
	// -------------------------------------------
	...
	{
		if( QuadraticInteger::d == 2 && aiDenominator->bIsEqualTo( QuadraticInteger( 4, -2 ) ) )
			return 8;
		else if( QuadraticInteger::d == 5 && aiDenominator->bIsEqualTo( QuadraticInteger( 8, -4 ) ) )
			return 5;
	...
	{
		if( ( aiNumerator->bIsEqualTo( QuadraticInteger( 1, 1 ) ) && aiDenominator->bIsEqualTo( QuadraticInteger( 0, 2 ) ) ) )
			return 8;
	}
	else if( QuadraticInteger::d == 3 )
	{
		if( aiNumerator->bIsEqualTo( QuadraticInteger( 7, -4 ) ) && aiDenominator->bIsEqualTo( QuadraticInteger( 28, -16 ) ) )
			return 3;
		else if( aiNumerator->bIsEqualTo( QuadraticInteger( 2, -1 ) ) && aiDenominator->bIsEqualTo( QuadraticInteger( 4, -2 ) ) )
			return 4;
		else if( ( aiNumerator->bIsEqualTo( QuadraticInteger( 2, -1 ) ) && aiDenominator->bIsEqualTo( QuadraticInteger( 28, -16 ) ) )
			|| ( aiNumerator->bIsEqualTo(1) && aiDenominator->bIsEqualTo( QuadraticInteger(8, -4) )
			)
		)
			return 12;
	}
	...
	{
		if( aiNumerator->bIsEqualTo( QuadraticInteger( 1, 1 ) ) && aiDenominator->bIsEqualTo( 4 ) )
			return 5;
		else if( aiNumerator->bIsEqualTo( QuadraticInteger( 5, 5 ) ) && aiDenominator->bIsEqualTo( QuadraticInteger( 8, 4 ) ) )
			return 10;
		else if( aiNumerator->bIsEqualTo( QuadraticInteger( 2, 1 ) ) && aiDenominator->bIsEqualTo( 4 ) )
			return 10;
	}
	else if( QuadraticInteger::d == 7 )
	{
		if( aiNumerator->bIsEqualTo( QuadraticInteger( 127, -48 ) ) && aiDenominator->bIsEqualTo( QuadraticInteger( 508, -192 ) ) )
			return 3;
		else if( aiNumerator->bIsEqualTo( QuadraticInteger( 8, -3 ) ) && aiDenominator->bIsEqualTo( QuadraticInteger( 16, -6 ) ) )
			return 4;
	}
...
void QuadraticInteger_AlVin::findVector(AlgebraicInteger* aiX0, AlgebraicInteger* aiNorm2)
{
	...
	findVector( qi0, dynamic_cast< QuadraticInteger* >( aiNorm2 ), 1, qiSumComp, *qi0 );
}

void QuadraticInteger_AlVin::findVector(QuadraticInteger* qi0, QuadraticInteger* qiNorm2, unsigned int iIndex, QuadraticInteger qiSumComp, QuadraticInteger qiGCDComponents )
{
	...
	/*
	 * If d=1 (4), then iFirstMin=ymin, iFirstMax=ymax, iSecMin=xmin and iSecMax=xmax. In particular, we first fix y and then x.
	 * If d=2,3 (4), then iFirstMin=xmin, iFirstMax=xmax, iSecMin=ymin and iSecMax=ymax. In particular, we first fix x and then y.
	 */
	
	bool bAdmissible, bReturn( false );
	unsigned int j;
	long int iFirstMin, iFirstMax, iSec, iSecMin, iSecMax, iTemp, iTemp2;
	QuadraticInteger qi( 0 ), qiSquare( 0 ), qiTemp( 0 ), qiPartialNorm( 0 ), qiLastComponent( 0 ), qiConjSumComp( qiSumComp );
	...
	for( long int iFirst( iFirstMin ); iFirst <= iFirstMax && !bReturn; iFirst++ )
	{
		if( bIsOneMod4 )
		{
			...
			iSecMax = QuadraticInteger::iSQRT_quotient( qiSumComp, qiQF[iIndex] ) + 1 - ( (iTemp % 2) ? iTemp / 2 + 1 : iTemp / 2 );
			
			if( iComponentLessThan[ iIndex ] ) // k_iIndex <= k_(iIndex-1) ?
			{
				iTemp2 = qiVectorCurrent[ iComponentLessThan[ iIndex ] ].b - iFirst;
				if( iTemp2 >= 0 )
					iTemp = qiVectorCurrent[ iComponentLessThan[ iIndex ] ].a + ( iTemp2 + iSQRTsup<unsigned long int>( d * iTemp2 * iTemp2 ) ) / 2;
				else
				{
					iTemp2 = abs( iTemp2 );
					iTemp2 += iSQRT<unsigned long int>( iTemp2 * iTemp2 * d );
					
					iTemp = qiVectorCurrent[ iComponentLessThan[ iIndex ] ].a - ( (iTemp2 % 2) ? iTemp2 / 2 + 1 : iTemp2 / 2 );
				}
		...
		{
			iTemp = iSQRTQuotient( (unsigned long int)iFirst * iFirst, (unsigned long int)d );
			iSecMin = -iTemp - 1;
			iSecMax = QuadraticInteger::QuadraticInteger::iSQRT_quotient( qiSumComp, qiDQF[iIndex] ) - iTemp;
			...
			if( iComponentLessThan[ iIndex ] ) // k_iIndex <= k_(iIndex-1) ?
			{
				...
				if( iTemp2 >= 0 )
					iTemp = iSQRTQuotient<unsigned long int>( iTemp2 * iTemp2, d) + qiVectorCurrent[ iComponentLessThan[ iIndex ] ].b;
				else
					iTemp = -iSQRTsupQuotient<unsigned long int>( iTemp2 * iTemp2, d) + qiVectorCurrent[ iComponentLessThan[ iIndex ] ].b;
		...
		for( iSec = iSecMin; iSec <= iSecMax; iSec++ )
		{
			...
			// TODO: incorporer la valeur MAX pour les composantes suite à qiBilinearProducts
			if( iComponentLessThan[ iIndex ] && !qi.bIsLessOEThan( qiVectorCurrent[ iComponentLessThan[ iIndex ] ] ) )
				continue;
			...
			{
				// qiTemp = qiQF[iIndex] * ( qi - qiLastComponent ) * qiVectors[j + iDimension][iIndex]
				qiTemp.set( &qi );
				...
				if( qiBilinearProducts[j].bIsGreaterThan( 0 ) ) // qiVectorCurrent is not admissible
					bAdmissible = false;
			}
