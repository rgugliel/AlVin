...
RCyclotomic7Integer_VFs::RCyclotomic7Integer_VFs( vector< AlgebraicInteger* > aiPossibleNorms2, const RCyclotomic7Integer& rciAlpha0 )
: rciAlpha0( rciAlpha0 ), rciMinusAlpha0( rciAlpha0 ), AlVinFractions( aiPossibleNorms2 ), rciPossibleNorms2_max( dynamic_cast< RCyclotomic7Integer* >( aiPossibleNorms2[ aiPossibleNorms2.size() - 1 ] ) )
{
...
void RCyclotomic7Integer_VFs::computeNextAlVinFractions()
{
	...
	// ----------------------------------------
	// Bounds
	unsigned int iMin( iLastMaximum ), iSizeTemp( ( iMin + iBatchSize ) * ( iMin + iBatchSize ) - iLastMaximum ) ;
	...
	for( auto aiE : aiPossibleNorms2 )
	{
		...
		long int iMprime( 0 ), imprime( 0 ); // Used only if bIsMprimeLongInt/bIsmprimeLongInt is true
		...
		if( rciE->bIsLongInt() )
		{
			...
			iSqrtTemp = iSQRT< unsigned long int >( iTemp ); // rciE & iLastMaximum are positive
		...
		interval gaol_min( gaol_temp + gaol_Mprime * ( gaol_2 * ( gaol_l1 + gaol_l3 ) + gaol_3 * gaol_l2 )  );
		
		// Third term (max):
		interval gaol_max( gaol_temp * gaol_minus1 + gaol_mprime * ( gaol_2 * ( gaol_l1 + gaol_l3 ) + gaol_3 * gaol_l2 )  );
		...
		if( !gaol_min.is_an_int() || !gaol_max.is_an_int() )
			throw( string( "RCyclotomic7Integer_VFs::computeNextAlVinFractions() : Not an int (bounds for b)" ) );
		...
		for( long int b( floor( gaol_min.left() ) ); b <= bMax; b++ )
		{
			...
			gaol_min *= ( ( gaol_2 * ( gaol_l1 + gaol_l3 ) + gaol_l2 * gaol_3 ) * gaol_b + gaol_l3 * gaol_R2 - gaol_R3 - gaol_Mprime);
			gaol_max *= ( ( gaol_2 * ( gaol_l1 + gaol_l3 ) + gaol_l2 * gaol_3 ) * gaol_b - gaol_l3 * gaol_R2 + gaol_R3 - gaol_mprime);

			gaol_min = ceil( gaol_min / gaol_7 );
			gaol_max = floor( gaol_max / gaol_7 );	
			
			if( !gaol_min.is_an_int() || !gaol_max.is_an_int() )
				throw( string( "RCyclotomic7Integer_VFs::computeNextAlVinFractions() : Not an int (bounds for a)" ) );
			...
			for( long int a( floor( gaol_min.left() ) ); a <= aMax; a++ )
			{
				...
				{
					gaol_min = ( gaol_Mprime - gaol_a * gaol_l1 - gaol_b * gaol_l2 ) / gaol_l3;
					gaol_min = ceil( gaol_min );
					
					if( !gaol_min.is_an_int() )
						throw( string( "RCyclotomic7Integer_VFs::computeNextAlVinFractions() : Not an int (bound inf for c)" ) );
				...
				{
					gaol_max = ( gaol_mprime - gaol_a * gaol_l1 - gaol_b * gaol_l2 ) / gaol_l3;
					gaol_max = floor( gaol_max );
					
					if( !gaol_max.is_an_int() )
						throw( string( "RCyclotomic7Integer_VFs::computeNextAlVinFractions() : Not an int (bound sup for c)" ) );
				...
				for( long int c( cMin ); c <= cMax; c++ )
				{
					...
					if( gaol_rci.certainly_leq( gaol_mprime ) )
						continue;
					if( !gaol_mprime.certainly_le( gaol_rci ) ) // If we cannot decide if m' < k0
						throw( string( "RCyclotomic7Integer_VFs::computeNextAlVinFractions(): Impossible to compare" ) );
					
					rci.conjugate( 2 );
					gaol_temp = abs( rci.to_interval() );
					if( gaol_temp.certainly_ge( gaol_R2 ) ) 
						continue;
					if( !gaol_temp.certainly_leq( gaol_R2 ) ) // If we cannot decide if | sigma_2( k0 ) | < S_2
						throw( string( "RCyclotomic7Integer_VFs::computeNextAlVinFractions(): Impossible to compare" ) );
					...
					if( gaol_temp.certainly_ge( gaol_R3 ) ) 
						continue;
					if( !gaol_temp.certainly_leq( gaol_R3 ) ) // If we cannot decide if | sigma_3( k0 ) | < S_3
						throw( string( "RCyclotomic7Integer_VFs::computeNextAlVinFractions(): Impossible to compare" )  );
					...
					AlVinFraction* vf = new AlVinFraction( new RCyclotomic7Integer( rci ), new RCyclotomic7Integer( *rciE ), rciNum );
